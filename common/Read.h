...
int Read(const char *filename, vector<pair<uint32_t,uint32_t>> &Datastream, int &flownum)
{
 ...
 //   vector<pair<uint32_t,uint32_t>> Datastream;
    ...
    cout<<"data item: "<<count<<endl;
    cout<<"real cardinality: "<<fe.size()<<endl;
    return count;
}
